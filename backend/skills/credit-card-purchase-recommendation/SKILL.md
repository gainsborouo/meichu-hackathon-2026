...
- explain each choice;
- verify each choice against the bank's official page (this only checks existing candidates).
...
6. **Official sources only for "verified".** Follow `official-source-policy.md`. Report
   only URLs on the bank's own domain in `official_sources`. If you found none, return an
   empty list; the backend will mark the pick unverified. Never claim verification you
   did not perform.
...
Search is for **verifying** that a candidate you were given is real and current on the
...
Use the `web_search` tool to confirm that a chosen campaign is real and current on the
bank's official site. A query may contain only: product name, store name, candidate card
